...
const PROJECT_TOOLS = [
  ...
  },
  {
    ...
    nativeOnly: true,
    description: "Execute one bounded TypeScript workflow against the audited Cyberboss NCP profile. Common operations are directly available without discovery: workspace_search({query,glob?,maxResults?}), workspace_read({path,startLine?,maxLines?}), workspace_git_status({mode:'status'|'diff',path?}), ops_query_logs({service?,since?,lines?}), workspace_apply_patch({path,expectedSha256,patch}), and workspace_run_tests({route:'syntax'|'node-test',files?}). Use Promise.all for independent operations and keep dependent search/read/edit/test steps in the same workflow. Garden operations are garden_get_my_status/get_self/get_machine/list_threads/get_thread/list_games/get_game_summary/get_chat_messages/list_activity; browser observations are playwright_browser_snapshot/browser_console_messages/browser_network_requests/browser_network_request/browser_get_config. NCP inherits the main model's authority for the current task when authorization is supplied; it cannot expand or reuse that authority. The boundary allows at most 8 calls and 4 concurrent calls and blocks arbitrary shell, filesystem, network, installation, scheduling, and internal lifecycle writers.",
